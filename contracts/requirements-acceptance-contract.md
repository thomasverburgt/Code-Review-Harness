...
Version: 1.0.0  
Decision: [ADR-0020](../adr/0020-human-acceptance-of-capability-requirements-artifact.md)

## Purpose

This contract governs the boundary between a technically valid CAP-REQ artifact and its use as an accepted capability input. It prevents the harness, a model, or a project maintainer from substituting for the external human requirements authority.
...
3. A `governance-records-verifier` independently verifies packet, artifact, authority, and record-integrity bindings.
4. The harness deterministically derives eligibility. Only `accept_review_as_complete` plus a verified record yields `eligible_for_accepted_capability_input`.
5. Every other disposition, failed verification, absent record, or revoked eligibility fails closed as not eligible.
