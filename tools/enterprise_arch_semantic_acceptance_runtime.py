#!/usr/bin/env python3
"""Record and derive the ADR-0035 project-owner ENT-ARCH semantic disposition."""
...
from accepted_live_enterprise_arch_runtime import validate_candidate
...
RUN = ROOT / "fixtures/enterprise-arch-admission/evidence/2026-08-02/adr0035/gx10-live-evaluation/corrected-run"
PACKET = RUN / "semantic-review-packet.json"
CANDIDATE = RUN / "ent-arch-model-candidate.artifact.json"
...
NAMESPACE = uuid.UUID("35000000-0000-4000-8000-000000000001")
DECIDED_AT = "2026-08-02T09:00:00Z"
DERIVED_AT = "2026-08-02T09:01:00Z"
...
def _authority(action: str) -> None:
    ...
    if not owner or action not in owner["allowed_actions"] or "HUMAN-PROJECT-OWNER-001" not in owner["human_subject_ids"]:
        raise ValidationFailure("project owner lacks ENT-ARCH semantic authority")


def load_exact_review() -> tuple[dict[str, Any], dict[str, Any]]:
    packet, candidate = load_json(PACKET), load_json(CANDIDATE)
    gate, source = load_json(GATE), load_json(SOURCE)
    assert_schema(packet, "enterprise-architecture-semantic-review-packet.schema.json", "ENT-ARCH semantic packet")
    if content_hash({**packet, "packet_hash": None}) != packet["packet_hash"]:
        raise ValidationFailure("ENT-ARCH semantic packet hash mismatch")
    ...
    if (packet["candidate"]["id"], packet["candidate"]["hash"]) != (
        ...
        raise ValidationFailure("ENT-ARCH semantic packet candidate binding mismatch")
    return packet, candidate


def build_disposition() -> dict[str, Any]:
    packet, candidate = load_exact_review()
    _authority("accept_enterprise_architecture_semantics")
    value = {
        ...
        "semantic_judgment": {"statement": "Semantic disposition passes.",
                              "architecture_coherence": "insufficient_evidence",
                              "confidence_interpretation": "confidence_in_unassessability_not_architecture_readiness"},
        "acknowledged_boundaries": {"single_capability_only": True, "no_cross_capability_fitness": True,
                                    "no_architecture_approval": True, "no_risk_acceptance": True,
                                    "no_requirements_satisfaction": True, "comparison_only": True},
    ...
    assert_schema(value, "enterprise-architecture-semantic-disposition.schema.json", "ENT-ARCH semantic disposition")
    return value


def validate_disposition(disposition: dict[str, Any]) -> None:
    packet, candidate = load_exact_review()
    assert_schema(disposition, "enterprise-architecture-semantic-disposition.schema.json", "ENT-ARCH semantic disposition")
    ...
    if actual != expected or content_hash({**disposition, "disposition_hash": None}) != disposition["disposition_hash"]:
        raise ValidationFailure("ENT-ARCH semantic disposition binding or hash mismatch")
    _authority("accept_enterprise_architecture_semantics")
...
def build_finalization(disposition: dict[str, Any]) -> dict[str, Any]:
    ...
    value = {
        ...
        "target_record_type": "enterprise_architecture_semantic_disposition",
    ...
    assert_schema(value, "project-owner-finalization.schema.json", "ENT-ARCH semantic finalization")
    return value


def validate_finalization(disposition: dict[str, Any], finalization: dict[str, Any]) -> None:
    validate_disposition(disposition)
    assert_schema(finalization, "project-owner-finalization.schema.json", "ENT-ARCH semantic finalization")
    if (finalization["target_record_type"], finalization["target_record_id"], finalization["target_record_hash"]) != (
            "enterprise_architecture_semantic_disposition", disposition["disposition_id"], disposition["disposition_hash"]):
        raise ValidationFailure("ENT-ARCH semantic finalization binding mismatch")
    if content_hash({**finalization, "finalization_hash": None}) != finalization["finalization_hash"]:
        raise ValidationFailure("ENT-ARCH semantic finalization hash mismatch")
    if finalization["finalization_state"] != "finalized" or not all(finalization["checks"].values()):
        raise ValidationFailure("ENT-ARCH semantic finalization did not pass")
...
def derive_eligibility(disposition: dict[str, Any], finalization: dict[str, Any], *, revoked: bool = False) -> dict[str, Any]:
    ...
    value = {
        ...
        "state": "eligible_for_enterprise_synthesis_candidate_evaluation" if eligible else "not_eligible",
        ...
        "ent_arch_scheduled": False, "ent_synth_scheduled": False,
        "report_effect": "none", "deployment_effect": "none", "record_hash": "sha256:" + "0" * 64,
    }
    _hashed(value, "record_hash")
    assert_schema(value, "enterprise-architecture-semantic-eligibility.schema.json", "ENT-ARCH semantic eligibility")
    return value
...
def persist_records(root: Path, disposition: dict[str, Any], finalization: dict[str, Any], eligibility: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ledger.persist_record(f"enterprise-architecture-dispositions/{disposition['disposition_id']}.json", "enterprise-architecture-disposition", disposition["disposition_id"], disposition),
        ...
        ledger.persist_record(f"enterprise-architecture-eligibility/{eligibility['eligibility_id']}.json", "enterprise-architecture-eligibility", eligibility["eligibility_id"], eligibility),
    ]
