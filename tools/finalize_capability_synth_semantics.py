#!/usr/bin/env python3
"""Materialize the exact ADR-0034 project-owner semantic disposition records."""
...
from capability_synth_semantic_acceptance_runtime import build_records, persist_records
...
def main() -> int:
    ...
    for name, value in (("semantic-disposition.json", disposition), ("project-owner-finalization.json", finalization),
                        ("enterprise-evaluation-eligibility.json", eligibility)):
        (output / name).write_bytes(pretty_bytes(value))
    refs = persist_records(output / "ledger", disposition, finalization, eligibility)
    summary = {"status": "owner_finalized", "adr": "ADR-0034", "disposition": disposition["disposition"],
               ...
               "eligibility_state": eligibility["state"], "cap_synth_scheduled": False,
               "enterprise_scheduled": False, "report_effect": "none", "deployment_effect": "none",
               "ledger_hashes": [item["object_hash"] for item in refs]}
