#!/usr/bin/env python3
"""Run isolated GX-10 Qwen3-32B calibration for ADR-0039 Increment 2."""
...
from run_specialist_build_supply_chain_increment2 import CONFIG, GENERATED_AT, build_candidate, build_manifest, locator
...
ROLE_SUITE = "adr0039-increment2-build-supply-chain-live-calibration"
ALL_SUITE = "adr0039-increment2-all-build-supply-chain-specialists"


def safe_error_text(exc: Exception, api_key_env: str) -> str:
    """Return a bounded diagnostic with credentials and bearer material removed."""
    message = str(exc)
...
def run_role(designation: str, output: Path, base_url: str, api_key_env: str, timeout: int) -> dict:
    ...
    evidence = [{"evidence_id": evidence_id, "path": path, "line": line, "section": section, "safe_excerpt": text, "content_hash": file_hash} for evidence_id, path, line, text, section, file_hash, _source_type in cfg["evidence"]]
    context = {
        "designation": designation, "generation_contract": schema,
        "authoritative_focus_binding": manifest["focus_binding"], "immutable_repository": manifest["repository"],
        "admitted_evidence": evidence,
        "required_result_shape_and_bounded_facts": deterministic["extensions"]["specialist"]["role"],
        "constraints": [
            "Return only the strict role payload.", "Use only admitted evidence IDs and preserve missing evidence as unknown.",
            "Keep definitions, built or executed artifacts, deployment state, and observed runtime state separate.",
            "Do not invent image builds, pipeline runs, releases, IaC plans, applies, drift checks, cloud state, or approvals.",
            "Keep the role comparison-only, unscheduled, downstream-ineligible, and human-authority bounded.",
            "Human review is deferred; structural validation is not semantic approval.",
        ],
    }
    model = load_json(model_path)
    adapter = VLLMAdapter(base_url, model["model_id"], api_key_env=api_key_env, max_tokens=model["max_output_tokens"], disable_thinking=True, use_json_schema=True, guided_array_max_items=12)
    target = output / cfg["slug"]
    target.mkdir(parents=True, exist_ok=True)
    summary = {"designation": designation, "suite": ROLE_SUITE, "passed": False, "candidate_status": "candidate", "semantic_acceptance": "human_review_deferred", "human_review_deferred": True, "scheduled": False, "product_fan_in_eligible": False, "report_eligible": False, "deployment_authorized": False, "a100_production_authorized": False, "test_platform": "GX-10 / approved DGX Spark-equivalent"}
    try:
        ...
        for name, value in (("input-manifest.json", manifest), ("model-candidate.artifact.json", candidate), ("evidence-locators.json", locators), ("human-shadow-review-packet.json", packet)):
            atomic_write(target / name, pretty_bytes(value))
        summary.update({"passed": True, "status": "complete", "raw_response_hash": raw_hash, "artifact_id": candidate["artifact"]["artifact_id"], "artifact_hash": content_hash(candidate), "packet_id": packet["packet_id"], "packet_hash": packet["packet_hash"], "usage": usage})
...
def main() -> int:
    ...
    summary = {"suite": ALL_SUITE, "passed": all(item["passed"] for item in results), "results": results, "human_review_deferred": True, "scheduled": False, "product_fan_in_eligible": False, "report_eligible": False, "deployment_authorized": False, "a100_production_authorized": False}
