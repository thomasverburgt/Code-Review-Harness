...
Current state: `blocked`. ADR-0016 semantic adjudication awaits an external human decision. The ADR-0020 requirements response has been recorded but awaits independent verification. ENT-ARCH, ENT-GOV, ENT-STRAT, and ENT-SYNTH have protocol calibration evidence but not accepted-live multi-domain semantic evaluations. No candidate is scheduled. See the living [`current-state` projection](../../status/current-state.md); retained readiness manifests remain creation-time snapshots.
